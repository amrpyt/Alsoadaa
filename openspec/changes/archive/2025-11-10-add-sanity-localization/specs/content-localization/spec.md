# Capability: Content Localization

## Overview

The Content Localization capability enables dynamic management of multilingual UI text through Sanity CMS, allowing non-developers to update translations without code changes.
...
- Translation fetch time: < 100ms (p50), < 200ms (p95)
- Fallback activation time: < 10ms
- Zero visible errors during Sanity downtime
- Content team can update translations independently in < 2 minutes
- AI translation accuracy: > 85% (manual review)
- Migration success rate: 100% (all 471 translations)
...
None (first localization capability)
